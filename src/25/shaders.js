...
export const Shaders = (li) => {
    ...
    const fragment = `
        ...
        fn main(input: Input) -> [[location(0)]] vec4<f32> {
            let N<f32> = normalize(input.vNormal.xyz);                
            let L<f32> = normalize(uniforms.lightPosition.xyz - input.vPosition.xyz);     
            let V<f32> = normalize(uniforms.eyePosition.xyz - input.vPosition.xyz);          
            let H<f32> = normalize(L + V);
            ...
            let finalColor<f32> = input.vColor * (ambient + diffuse) + vec3<f32>${li.specularColor}*specular; 
