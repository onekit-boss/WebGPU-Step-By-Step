...
export const Shaders = (li) => {
    ...
    const fragment = `
        ...
        fn main([[location(0)]] vPosition: vec4<f32>, [[location(1)]] vNormal: vec4<f32>) ->  [[location(0)]] vec4<f32> {
            let N<f32> = normalize(vNormal.xyz);                
            let L<f32> = normalize(uniforms.lightPosition.xyz - vPosition.xyz);     
            let V<f32> = normalize(uniforms.eyePosition.xyz - vPosition.xyz);          
            ...
            let finalColor<f32> = vec3<f32>${li.color}*(ambient + diffuse) + vec3<f32>${li.specularColor}*specular; 
